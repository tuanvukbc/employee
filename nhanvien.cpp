...
void Other::Printfcsv(Employee p)
{
        ofstream myfile(this->file, ios::app);
        myfile << p.getid() << ',' << p.getname() << ',' << p.getbirth() << ',' << p.getadress() << ',' << p.getteam();

        myfile.close();
}
void Other::PrintText(Employee p, int n = 0)
{
        ofstream myfile(this->file, ios::app);
        myfile << p.getid() << ',' << p.getname()<< ',' << n;
        if (n != 0)
        {
                for (int i = 0; i < n; i++)
                {
